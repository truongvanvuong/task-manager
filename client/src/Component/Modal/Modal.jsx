...
const Modal = ({ open, setOpen, titleModal, mode, taskData, refreshData }) => {
    ...
    const handleOk = async () => {
        ...
        if (!hasError) {
            const url = `${BASE_URL}/task${mode === 'edit' ? `/${taskData._id}` : ''}`;
    ...
    return (
        <ModalAntd
            title={<h1 className="text-[1.4rem] font-medium dark:text-textHeaddingDark">{titleModal}</h1>}
            centered
            open={open}
            onOk={handleOk}
        ...
        >
            <div>
                ...
                <div className="mt-6">
                    <label htmlFor="important" className="text-[1rem] cursor-pointer hover:opacity-90">
                        Quan trọng
                    </label>
                    <Checkbox
